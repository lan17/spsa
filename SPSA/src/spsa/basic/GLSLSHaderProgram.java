...
/**
 * <p>
 * Title: Scientific Volume Rendering
 * </p>
 * 
 * <p>
 * Description: Lev Neiman's Summer Job
 * </p>
 * 
 * <p>
 ...
 * 
 * <p>
 * Company: Dr. Peter Jung
 * </p>
 * 
 * @author Lev A. Neiman
 * @version 1.0
 */
public class GLSLSHaderProgram implements ShaderProgram
{
	...
	public void addShader(GLAutoDrawable g, ProgramType type, String[] source) throws IllegalArgumentException
	{
		GL gl = g.getGL();
		// printStringArr( source );
		if (source == null) { throw new IllegalArgumentException( "Wrong arguments passed to addShader( int, String[] )" ); }
	...
	public static String[] trippy = { "varying vec4 colorz; void main( void ) { float f; vec3 norm = normalize( gl_NormalMatrix * gl_Normal ); vec3 lightv = normalize( gl_LightSource[0].position - gl_ModelViewMatrix * gl_Vertex );   const vec4 color1 = { 0,1,0, 1 };   const vec4 color2 = { 1,0,0,1 };   const vec4 color3 = { 0, 0, 1, 1 };   float dt = dot(lightv, norm );   if( dt < .3 ) gl_FrontColor = color1;      else if( dt < .7 ) gl_FrontColor=color2;         else gl_FrontColor = color3;   gl_FrontColor *= vec4( norm, 1 ) ;   colorz = gl_FrontColor;   gl_Position = gl_ModelViewProjectionMatrix * gl_Vertex;}" };
	public static String[] simple_fragment_shader = { "varying vec4 colorz;void main(void){gl_FragColor = colorz;}" };

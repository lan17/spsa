        ...
        extends ANode
{
    ...
    private static void buildMesh()
    {
        ...
        for( double i = 0f; i < 2 * PI; i += detail )
        {
            ...
            Coordinate b = new Coordinate( Math.cos( i ) * radius,
                                           Math.sin( i ) * radius, thickness );
            Coordinate c = new Coordinate( Math.cos( i + detail ) * radius,
                                           Math.sin( i + detail ) * radius,
                                           thickness );
            ...
            Coordinate normaln = new Coordinate( Math.cos( i ), Math.sin( i ),
                                                 0 );
    ...
    public synchronized void render( GLAutoDrawable panel )
    {
        ...
        if( need_to_build_mesh )
        {
            ...
            System.out.println( "ARE VERTEX ARRAYS SUPPORTED? " +
                                Boolean.toString( vertex_arrays_supported ) );
            if( !vertex_arrays_supported )
            {
                detail = .1f;
            }
        ...
        if( show )
        {
            ...
            if( vertex_arrays_supported && use_vertex_arrays )
            {
                ...
                for( Double s : slices )
                {
                    ...
                    gl.glDrawArrays( GL.GL_TRIANGLES, start,
                                     share.intValue() + 3 );
            ...
            if( !vertex_arrays_supported || !use_vertex_arrays )
            {
                for( int i = 0; i < mesh.size(); i += 2 )
                {
                    ...
                    gl.glBegin( gl.GL_TRIANGLES );
                    {
                        gl.glNormal3d( normal.getX(), normal.getY(),
                                       normal.getZ() );

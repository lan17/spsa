 ...
 * 
 * <p>
 * Description: container for 3D coordinate.
 * </p>
 * 
 * <p>
 ...
 * @version 1.0
 */
public class Coordinate implements Serializable, Comparable, Cloneable
{
	/**
	...
	/*
	 * public static void roundOff( Coordinate a, int dec_places ) { a.x =
	 * Func.roundOff( a.x, dec_places ); a.y = Func.roundOff( a.y, dec_places );
	...
	public synchronized void multiply4x4(final double[] matrix) throws IllegalArgumentException
	{
		try
		{
	...
	public static Coordinate multiply4x4(Coordinate a, final double[] matrix) throws IllegalArgumentException
	{
		Coordinate ret = new Coordinate();
		if (matrix.length != 16) { throw new IllegalArgumentException( "Passed matrix is not 4x4" ); }
	...
	public int compareTo(Object o)
	{
		if (o instanceof Coordinate)
		{
			...
			// if( Math.abs( this.x - c.x ) <= _epsilon && Math.abs( this.y -
			// c.y ) <= _epsilon && Math.abs( this.z - c.z ) <= _epsilon )
			// if( x == c.x && y == c.y && z == c.z )
			// c.print();
			if (Math.abs( x - c.x ) > _epsilon) { return Double.compare( x, c.x ); }
			if (Math.abs( y - c.y ) > _epsilon) { return Double.compare( y, c.y ); }
			if (Math.abs( z - c.z ) > _epsilon) { return Double.compare( z, c.z ); }
		}
		return 0;
	}

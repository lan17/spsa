...
/**
 * <p>
 * Container for data loaded from OBJ and MTL files. It prepares it for use in
 * ModelNode.
 * </p>
 * 
 * <p>
 * Description: Container for data loaded from OBJ and MTL files. It prepares it
 * for use in ModelNode.
 * </p>
 * 
 * <p>
...
public class ModelFile
{
	 ...
	 * @param a
	 *            String - input string.
	 * @return String - substring that is from begining of input to the last
	 *         occurrence of '.'
	 */
	...
	private void init()
	{
		...
		// file_name = System.getProperty( "user.dir" ) + separator +
		// data_folder + separator + name + ".mtl";
		...
		// System.out.println( "OBJ File - " + mdf.toString() + " exists? - " +
		// mdf.exists() );
		try
		{
			data = new Scanner( mdf );
	...
	/**
	 * this method associates Material names to Vector's of triangles that
	 * belong to that material group.
	 */
	private void sortMaterials()
	{
		...
		for (Triangle triangle : triangles)
		{
			...
			Vector< Triangle > group = null;
			group = mtl_groups.get( triangle.material.name );
			if (group == null)
			{
	...
	private void readMTL(Scanner s) throws ModelFile_ReadError
	{
		...
		try
		{
			...
			while (s.hasNext())
			{
				...
				line_string.trim();
				Vector< String > line = Utils.splitString( line_string, ' ' );
				...
				if (line.size() > 0)
				{
					...
					if (line.get( 0 ).equals( "newmtl" ))
					{
						...
						// System.out.println( "Reading in new material - " +
						// line.get( 1 ).trim( ) );
					}
			...
			while (input.hasNext())
			{
				...
				if (line.size() > 1)
				{
					...
					// f stands for face, so its time to create a Triangle
					{
						...
						if (Utils.splitString( line.get( 1 ), '/' ).size() == 1)
						{
						...
						{
							a = Integer.valueOf( Utils.splitString( line.get( 1 ), '/' ).get( 0 ) );
							b = Integer.valueOf( Utils.splitString( line.get( 2 ), '/' ).get( 0 ) );
							c = Integer.valueOf( Utils.splitString( line.get( 3 ), '/' ).get( 0 ) );
							texturez = true;
						}
						...
						// if( tmp.material == null ) { System.out.println(
						// "FUCK FUCK FUCK " + tmp.material_name ); }
	 ...
	 * 
	 * @param name
	 *            String name of the model to load
	 */

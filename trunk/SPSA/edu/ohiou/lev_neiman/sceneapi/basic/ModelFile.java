...
import edu.ohiou.lev_neiman.sceneapi.basic.exceptions.ModelFile_ReadError;
...
public class ModelFile
{
    ...
    public Hashtable<String,
            Material> materials = new Hashtable<String, Material> ();
     ...
     */
    public Hashtable<String,
            Vector<Triangle>> mtl_groups = new Hashtable<String,
                                           Vector<Triangle>> ();
    ...
    public static Vector<String> expand( final String string, char delimeter )
    {
        ...
        {
            if( i > 0 && string.charAt( i ) == delimeter &&
                string.charAt( i - 1 ) != delimeter )
            {
     ...
     */
    private void readMTL( Scanner s )
            throws ModelFile_ReadError
    {
                ...
                if( line.size() > 0 )
                {
                    ...
                    if( line.get( 0 ).equals( "newmtl" ) )
                    {
                        if( cur_mtl != null )
                        {
                            ...
                            System.out.println( "added new mtl - " +
                                                cur_mtl.name.trim() + " :: " +
                                                cur_mtl.toString() );
                        }
                        cur_mtl = null;
                    ...
                    {
                        cur_mtl.texture = TextureManager.getTexture( line.get(
                                1 ).
                                trim() );
                    }
     ...
     */
    private void readOBJ( Scanner input )
            throws ModelFile_ReadError
    {
        try
        {
            ...
            while( input.hasNext() )
            {
                ...
                if( line.size() > 1 )
                {
                    ...
                    // f stands for face, so its time to create a Triangle
                    {
                        ...
                        {
                            a = Integer.valueOf( expand( line.get( 1 ), '/' ).
                                                 get( 0 ) );
                            b = Integer.valueOf( expand( line.get( 2 ), '/' ).
                                                 get( 0 ) );
                            c = Integer.valueOf( expand( line.get( 3 ), '/' ).
                                                 get( 0 ) );
                            texturez = true;
                        }
            ...
            for( Triangle triangle : triangles )
            {
                triangle.normal = ANode.getNormal( triangle.a, triangle.b,
                        triangle.c );
            }
